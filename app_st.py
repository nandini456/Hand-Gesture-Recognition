...
cap_device = st.sidebar.selectbox("Select Camera Device", [0, 1], index=0)
cap_width = st.sidebar.slider("Camera Width", 640, 1920, 960)
cap_height = st.sidebar.slider("Camera Height", 480, 1080, 540)
use_static_image_mode = st.sidebar.checkbox("Static Image Mode", value=False)
...
# Initialize FPS calculator and gesture history
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if hand_sign_id == 2:  # Assume "point" gesture
                point_history.append(landmark_list[8])  # Track index finger tip
            else:
                point_history.append([0, 0])  # Null point if no gesture detected
        ...
        point_history.append([0, 0])  # Append null points if no hand detected

    # Draw point history and info
    debug_image = draw_point_history(debug_image, point_history)
    debug_image = draw_info(debug_image, fps, mode=0, number=-1)
